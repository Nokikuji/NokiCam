...
# Steps occupy y=131 to y=131+6*26+22 = 309
...
function Set-Step([int]$Idx, [string]$State) {
    switch ($State) {
        ...
        "ok"     { $StepIco[$Idx].Text="✓"; $StepIco[$Idx].ForeColor=$GREEN;  $StepLbl[$Idx].ForeColor=$TXT }
...
function Invoke-Setup([string]$Exe, [string]$CmdArgs) {
    ...
    $proc.WaitForExit()
    $rest = $stdout.ReadToEnd(); if ($rest) { $rest.Split("`n") | ForEach-Object { Write-Log $_.TrimEnd("`r") } }
    $rest = $stderr.ReadToEnd(); if ($rest) { $rest.Split("`n") | ForEach-Object { Write-Log ("[!] " + $_.TrimEnd("`r")) } }
